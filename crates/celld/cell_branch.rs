...
//! Generic LTX cell branch from a parent version bucket (D1-BRANCH-RPC base.json).
...
pub fn ltx() -> Result<Arc<LtxRepl>, String> {
    LTX.get()
        .cloned()
        .ok_or_else(|| "cell branch requires fleet replication".to_string())
}
...
/// Project id used by branch CLIs' copy of [`validate_parent_bucket`].
...
pub fn project_from_cellp_bucket(spec: &str) -> Option<String> {
...
pub fn prepare(
    ...
    request: BranchRequest,
    sqlite_vec: bool,
) -> Result<(PreparedBranch, ReopenSpec), BranchFailure> {
    let started = std::time::Instant::now();
    let ltx = ltx().map_err(branch_error)?;
    let project_id = runtime_project_id().map_err(branch_error)?;
    validate_parent_bucket(&request.parent_bucket, &project_id).map_err(branch_error)?;
    let epoch = storage::activation_epoch(scope)
        .ok_or_else(|| branch_error(format!("no active database for {scope}")))?;
...
pub async fn branch_prepared(prepared: PreparedBranch) -> Result<BranchOk, BranchFailure> {
    let PreparedBranch {
        scope,
        request,
        epoch,
    ...
    let project_id = runtime_project_id().map_err(branch_error)?;
    let parent = validate_parent_bucket(&request.parent_bucket, &project_id).map_err(branch_error)?;
    let result = ltx
        .branch_from_parent(&scope, epoch, &parent, request.parent_epoch)
        .await
        .map_err(|error| branch_error(error.to_string()))?;
    Ok(BranchOk {
...
pub(crate) fn reopen(spec: &ReopenSpec) -> Result<(), BranchFailure> {
...
fn branch_error(message: impl Into<String>) -> BranchFailure {
    BranchFailure {
        message: message.into(),
    }
}

/// Read the LTX base pointer for a branched cell at its active epoch.
pub async fn active_base_pointer(cell: &str) -> Result<Option<BasePointer>, String> {
    let ltx = ltx()?;
    ltx.active_base_pointer(cell)
        .await
